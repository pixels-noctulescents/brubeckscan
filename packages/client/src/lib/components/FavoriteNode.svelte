...
<div class="flex w-full flex-col" in:scale>
	<Module>
		<div class="flex w-full flex-col gap-1">
			...
			<div class="flex w-full items-center justify-between">
				<div class="flex w-full items-center justify-between">
					<span
						><a title={node.db.address} href={`/nodes/${node.db.address}`}
							>{format.ethAddress(node.stats.address)}</a
						></span
					>
					<div class="flex">
						<button
							class="w-6 text-gray-300 transition duration-100 ease-in hover:text-blue-500"
							on:click={async () => {
								await navigator.clipboard.writeText(node.stats.address);
							}}
						>
							<Icon icon="material-symbols:content-copy" width="26" />
						</button>
						<div class="flex justify-between">
							<div class="w-6 text-gray-300 transition duration-100 ease-in hover:text-blue-500">
			...
			<div class="flex h-max w-full items-center justify-between">
				...
				{#if node.stats.status}
					<p class="text-lg text-green-400">OK</p>
				{:else}
					<p class="text-lg text-red-500">KO</p>
				{/if}
			</div>
			...
			<form class="flex" method="POST" use:enhance>
				...
				<button formaction="?/deleteFavorite" class="w-full">
					<div
						class="flex w-full justify-end text-gray-300 transition duration-100  ease-in hover:text-red-500"
					>

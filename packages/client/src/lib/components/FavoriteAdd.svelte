    ...
    <Module>
        <div class="flex flex-col gap-4 w-full">
            <form class="flex w-full items-center justify-between gap-10" method="POST" use:enhance>
                ...
                <input class="focus:border-0 focus:bg-slate-50 w-full max-w-xl bg-transparent p-2" name="address" type="text" required bind:value={address} placeholder="Enter a node address">
                {#if result?.add}
                <div class="flex">

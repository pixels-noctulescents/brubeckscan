...
{#if $hasEthereumProvider != undefined}
	{#if !$user && $hasEthereumProvider}
		<div>
			<Button handle={handleConnect}>
				<div
					class="flex items-center gap-2 rounded-md border-2 border-slate-300 px-2 py-1 transition duration-150 ease-in hover:bg-white hover:shadow-sm dark:hover:bg-black"
				>
					<p>Connect</p>
	...
	{:else if $user}
		<div
			class="flex select-none items-center gap-4 py-1 text-neutral-400"
			title="User DATA Balance"
		>
